import { INITIAL_PARTICIPANTS_DUMMY } from "../../../tests/testData";
import { getLayoutIndex } from "../utils/layoutFuncs";
import { pinNameToIndex } from "../constants/imageConstants";
...
export const STREAMS_ORDER = pexipDataJsonVar?.streamsOrder
  ...
  : [];
export const ROLE_STATUS = true; //ENV === ENVIRONMENT.dev ? true : false;
...
export const CONTROL_ROOM_PINS =
  ENV === ENVIRONMENT.dev
    ? 3
    : CONTROL_ROOM_SHOW_REFRESH
    ? pinNameToIndex(pexipDataJsonVar?.controlRoomData?.pinning_config)
    : pinNameToIndex(
...
export const LABEL_NAMES = {
  ...
  removePplErrMsg1: "Please remove",
  removePplErrMsg2: "presenters from the Stage to apply the",
  removePplErrMsg3: "layout.",
  adaptiveLayoutErrMsg:
    "The Adaptive Layout is only applicable for voice-activated on.",
  applyTransformLayoutErrMsg: "Error applying new layout: ",
  applyTransformLayoutSameLayout:
    "New layout is the same as the current layout.",
